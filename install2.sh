...
installpkg(){ apt-get install -S "$1" >/dev/null 2>&1 ;}
...
apt install --assume-yes dialog || error "Are you sure you're running this as the root user, are on an Arch-based distribution and have an internet connection?"
